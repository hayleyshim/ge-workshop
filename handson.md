...
**핵심 용어: Gemini Enterprise에서 데이터 스토어 라는 용어는 커넥터 또는 데이터 커넥터 라는 용어와 서로 바꿔서 사용할 수 있습니다.**
...
이제 진짜 핵심입니다 — Gemini가 **우리 회사 고유의 데이터(사규·거래처 정보 등)에 닿게** 하는 법. 여기서 쓰는 게 바로 **커넥터(데이터스토어, 도구)** 입니다. 
...
```
기업 공시 도구로 엔비디아 최근 30일 공시를 날짜,유형과 함께 알려줘.
```
...
> ✅ 결과 — 오늘·어제 날짜의 공시가 유형(10-Q·8-K 등)과 함께 뜨면 OK. 어떤 사내 문서에도 안 적힌, **시스템에서 실시간으로 가져와야만 알 수 있는 값**이라는 게 사내 지식과 다른 점이에요. ⚠️ "찾을 수 없음" → 회사명 표기 확인(비상장·외국계는 조회 불가).

<img src="images/06-connector-custom-mcp.png" alt="Custom MCP 서버 결과 화면" style="max-width:100%;border:1px solid #E2E8F0;border-radius:8px;margin:.6em 0">
...
> 🔗 **'사규'를 에이전트에 연결합니다.** 이 도구는 우리 회사 **사규가 들어간 데이터 스토어**예요(2-1에서 직접 써 본 그것). 2-1에선 매번 켜서 물었지만, 이제 **정책 질문이면 항상 사규를 검색해 출처와 함께 답하는 전용 비서**인 겁니다. 즉 이 에이전트의 답은 전부 **연결된 사규 데이터 스토어**에서 나옵니다.
...
### 3-2. (선택·심화) 미팅 사전 브리핑 에이전트 — 도구 여러 개 묶어보기
