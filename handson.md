...
| 14:50–15:10 | 2. 커넥터로 데이터 연결 — 사내 지식 + 기업 공시 직접 질의 | 커넥터·도구 감 잡기 & 사규·실시간 공시를 직접 검색 |
...
1번에서는 Gemini와 가볍게 대화해 봤죠. 이제 진짜 핵심입니다 — Gemini가 **우리 회사 고유의 데이터(사규·거래처 정보 등)에 닿게** 하는 법. 여기서 쓰는 게 바로 **커넥터(도구)** 입니다. 핵심은 딱 두 가지만 기억하면 됩니다.
...
> 본격적으로 만들기 전에, **이미 완성된 에이전트가 얼마나 강력한지** 진행자가 화면으로 보여드립니다. 직접 따라 할 필요 없이 **결과만 눈으로 보시면 돼요.** (조사형 에이전트는 시간이 걸려서, 미리 돌려둔 결과로 보여드립니다.) 좌측 메뉴 **에이전트 → 모든 에이전트 보기**에 다 들어 있어요.
...
> 🧩 **2-2에서 직접 써 본 그 '기업 공시 도구'(= Custom MCP 서버)** 를 이번엔 **에이전트에 붙박이로 연결**합니다. 2-2에선 매번 켜서 물었지만, 이제 **회사 조회 질문이면 알아서 이 도구로 답하는 전용 비서**로 굳히는 거예요. 똑같은 방식으로 **국내 DART·사내 ERP·CRM·재고 시스템**도 붙일 수 있습니다 — 이게 "확장 가능한 AI 동료"의 핵심입니다.
...
*▲ '+ 새 에이전트'를 누르면 나오는 화면. 여기서 이름·역할을 적고 도구(Custom MCP 서버)를 붙입니다.*
...
> 🎯 한 줄 정리 — **사내 지식 = "우리 문서에서 찾아 줘"**, **Custom MCP 서버= "외부 시스템 두드려 실시간으로 가져와 줘"**. 같은 에이전트가 질문에 따라 알맞은 손발을 골라 씁니다.
