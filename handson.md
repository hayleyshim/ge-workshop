...
### 2-2. 기업 공시 도구(Custom MCP)에 직접 물어보기
...
> 💡 엔비디아 대신 애플·마이크로소프트·테슬라 등 **미국 SEC에 상장된 회사** 아무거나 가능. 비상장·미국 외 미등록 기업은 조회되지 않습니다. 첫 호출이 5~10초 느린 건 정상이에요(잠자던 서버가 깨어나는 '콜드 스타트').

**2-2-3.** 답에 **실제 공시 날짜·유형**이 보이면 성공. 이건 일반 웹 검색이 아니라, 진행자가 미리 **Cloud Run에 올려 둔 우리 전용 MCP 서버**가 미국 SEC 전자공시(EDGAR)를 **실시간 호출**해 가져온 값이에요.
...
> 🎯 **두 도구를 다 써 봤어요** — 📚 **사내 지식**(우리 문서에서 찾기)과 🧩 **기업 공시**(외부 시스템에서 실시간으로 가져오기). 3번에서는 이 도구들을 **에이전트에 붙박이로 연결**해, 매번 켜지 않아도 알아서 골라 쓰는 **나만의 비서**를 만듭니다.
...
### 3-0. 먼저 구경 — 구글이 미리 만든 '완성형' 에이전트 *(진행자 시연)*
