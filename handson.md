...
| 14:40–14:50 | 1. 아이스브레이킹 — 인사 나누기 & 내 문제 등록 | 첫 대화 감각 + **내 업무 페인포인트 1개** |
| 14:50–15:10 | 2. 커넥터로 데이터 연결 — 사내 지식 + 기업 공시 직접 질의 | 커넥터·도구 사용 & 사규·실시간 공시를 직접 검색 |
| 15:10–15:50 | 3. 에이전트 — 완성형 시연(진행자) + 나만의 에이전트 만들기 ★ | 직접 만든 에이전트 + **내 문제에 적용해본 경험** + 팀 공유 |
| 15:50–16:00 | 4. 정리 | 우리 회사에 가져갈 첫 아이디어 1줄 |
...
> 💡 아직 답이 완벽하지 않아도 됩니다. 3. 에이전트 생성하기 실습에서 다시 꺼내 실제 에이전트로 만들어볼 거예요. **오늘 워크샵이 끝나면 여러분 손에 '내 문제를 푸는 에이전트 초안' 하나가 남습니다.**

---

## 2. 커넥터로 회사 데이터에 연결하기

이제 진짜 핵심입니다 — Gemini가 **우리 회사 고유의 데이터(사규·거래처 정보 등)에 닿게** 하는 법. 여기서 쓰는 게 바로 **커넥터(도구)** 입니다. 
...
에이전트에게 어떤 도구를 쥐여주느냐에 따라 할 수 있는 일이 달라져요. 오늘은 이런 도구들을 씁니다.
...
> 🧰 **잠깐 — 같은 '도구'지만 속은 3종류예요** *(처음엔 가볍게 읽고 넘어가도 됩니다)*
...
>
> 👉 셋 중 **'MCP 서버'는 기업 공시 하나뿐**입니다. 나머지 둘은 데이터 스토어와 기본 커넥터예요. 다만 **등록만 같은 메뉴('데이터 스토어')로** 하고, 에이전트엔 모두 **'도구(커넥터)'로 똑같이** 붙기 때문에 사용법은 동일합니다.
>
> 💬 **MCP(Model Context Protocol)이 뭐예요?** *'AI에게 외부 시스템(공시·ERP·CRM 등)을 연동하는 **표준 방식**'* 정도로만 알면 충분합니다. 오늘은 진행자가 미리 만들어 둔 걸 **고르기만** 하면 돼요.
...
### 2-2. 기업 공시 도구(Custom MCP 서버)에 직접 물어보기

사내 지식이 **우리 문서**를 찾아준다면, 이번엔 **외부 시스템에서 실시간으로** 값을 가져오는 도구를 써 봅니다. 채팅창에서 도구만 켜면 바로 체험돼요.

**2-2-1.** 채팅창 하단 **'원통' 아이콘**에서 **🧩 기업 공시 도구**만 **켭니다(ON)**. (사내 지식은 꺼두면 누가 답하는지 더 잘 보여요.)

**2-2-2.** 글로벌 상장사 이름을 넣어 물어보세요:

```
엔비디아 최근 공시 알려줘. 날짜랑 유형도 같이.
```

> 💡 엔비디아 대신 애플·마이크로소프트·테슬라 등 **미국 SEC에 상장된 회사** 아무거나 가능. 비상장·미국 외 미등록 기업은 조회되지 않습니다. 첫 호출이 5~10초 느릴 수 있어요.

**2-2-3.** 답에 **실제 공시 날짜·유형**이 보이면 성공입니다. 이건 일반 웹 검색이 아니라, 진행자가 미리 **Cloud Run에 올려 둔 우리 전용 MCP 서버**가 미국 SEC 전자공시(EDGAR)를 **실시간 호출**해 가져온 값이에요.

> ✅ **성공 신호** — 오늘·어제 날짜의 공시가 유형(10-Q·8-K 등)과 함께 뜨면 OK. 어떤 사내 문서에도 안 적힌, **시스템에서 실시간으로 가져와야만 알 수 있는 값**이라는 게 사내 지식과 다른 점이에요. ⚠️ "찾을 수 없음" → 회사명 표기 확인(비상장·외국계는 조회 불가).

> 🎯 **두 도구를 다 써 봤어요** — 📚 **사내 지식**(우리 문서에서 찾기)과 🧩 **기업 공시**(외부 시스템에서 실시간으로 가져오기). 3번에서는 이 도구들을 **에이전트에 연결**해, 매번 켜지 않아도 알아서 골라 쓰는 **나만의 비서**를 만듭니다.
...
> 본격적으로 만들기 전에, **이미 완성된 에이전트가 얼마나 강력한지** 진행자가 화면으로 보여드립니다. 직접 따라 할 필요 없이 **결과만 눈으로 보시면 돼요.** (조사형 에이전트는 시간이 걸려서, 미리 돌려둔 결과로 보여드립니다.) 
...
**② Idea Generation** — 주제를 주면 아이디어를 다량으로 만들고 **서로 겨루게 해 순위까지** 매겨줍니다.
...
> 진행자가 가상 회사 **한빛테크놀로지**의 사내 정책 샘플을 넣고 *"출장비 한도와 정산 기한"* 을 물으면, **출처(인용)와 함께** 답이 나옵니다. 그다음 버튼 하나로 **두 진행자가 자료를 대화로 풀어주는 팟캐스트(오디오 개요)** 까지 생성되는 걸 보여드릴게요.

> **연결 포인트** — 방금 본 건 **구글이 미리 완성해 둔** 에이전트예요. 지금부터는 **여러분이 직접** 코드 한 줄 없이 나만의 에이전트를 만들어 봅니다. NotebookLM처럼 자료를 매번 넣지 않아도, **사내 문서가 자동으로 연결된** 비서를 만드는 거예요.

<details>
<summary>🔧 <b>직접 써보고 싶다면</b> (워크샵 후 개인용 — 펼치기)</summary>

각 에이전트를 직접 돌려보고 싶을 때 쓰는 프롬프트입니다. Deep Research·Idea Generation은 채팅창 하단 **'원통' 아이콘 → Google 검색 ON** 후 사용하세요.
...
**NotebookLM** — 새 노트북 → 소스 추가 → 텍스트 붙여넣기에 아래 샘플을 넣고:
```
...
> **WOW 포인트** — 내일 미팅할 외부 회사를 채팅 한 줄로 **실시간 공식 데이터**로 파악합니다. 핵심은, 이게 일반 검색이 아니라 **우리가 직접 만들어 연동한 'Custom MCP 서버'** 가 미국 SEC 전자공시(EDGAR)를 실시간 호출해 가져온 값이라는 것 — 게다가 원본엔 없는 **위험도 점수**까지 계산해줍니다. *"회사가 원하는 시스템은 뭐든 이렇게 도구로 붙일 수 있구나"* 를 처음 체감하는 순간이에요.

> 🧩 **2-2에서 직접 써 본 그 '기업 공시 도구'(= Custom MCP 서버)** 를 이번엔 **에이전트에 연결**합니다. 2-2에선 매번 켜서 물었지만, 이제 **회사 조회 질문이면 알아서 이 도구로 답하는 전용 비서**인 거예요. 똑같은 방식으로 **국내 DART·사내 ERP·CRM·재고 시스템**도 연동할 수 있습니다 — 바로 "확장 가능한 AI 동료"의 핵심입니다.
...
*▲ '+ 새 에이전트'를 누르면 나오는 화면. 여기서 이름·역할을 적고 도구(Custom MCP 서버)를 연동합니다.*
...
> ⚠️ "찾을 수 없다" → 회사명 표기 확인(비상장·외국계는 조회 불가) / 첫 호출이 5~10초 느린 건 정상입니다.

> **느낀 점 한 줄** — *"검색이 아니라, 우리가 만든 MCP 서버가 진짜 오늘 공시로 답했어."* 이게 무료 챗봇과 결정적으로 다른 지점이에요.

#### 🆚 보너스 — 같은 에이전트에 '사내 지식'도 연결해보기 (선택, 3분)

방금 만든 에이전트에 `사내 지식` 도구를 **추가로** 연결해보세요 (도구 추가 → 사내 지식 체크). 이제 **외부(상대 회사)** 와 **내부(우리 사규)** 질문을 해보며 같은 에이전트가 어느 도구를 골라 쓰는지 관찰해 보세요.

```
엔비디아 어제 새로 올라온 공시 있어?
```
> → **기업 공시 도구(Custom MCP 서버)** 가 외부 시스템(SEC)에서 실시간으로 가져옵니다.
...
> 🎯 한 줄 정리 — **사내 지식 = "우리 문서에서 찾아 줘"**, **Custom MCP 서버= "외부 시스템 두드려 실시간으로 가져와 줘"**. 같은 에이전트가 질문에 따라 알맞은 도구를 골라 씁니다.
...
> 🔗 **이번엔 '사내 지식'을 에이전트에 연결합니다.** 이 도구는 우리 회사 **사규가 들어간 데이터 스토어**예요(2-1에서 직접 써 본 그것). 2-1에선 매번 켜서 물었지만, 이제 **정책 질문이면 항상 사규를 검색해 출처와 함께 답하는 전용 비서**인 겁니다. 즉 이 에이전트의 답은 전부 **연결된 사규 데이터 스토어**에서 나옵니다.
...
> **지금이 핵심입니다.** 1-4에서 적어둔 *내 업무의 반복 문제*를 꺼내, 에이전트로 설계해 봅니다. 남이 만든 시나리오를 따라 한 것과 **내 문제로 직접 만든 것**은 다음 주 월요일에 쓸 수 있느냐가 달라집니다.
...
> 여유가 있다면 도전해 보세요. **여러 도구를 한 에이전트에 묶으면** 어떤 일이 벌어지는지 보여주는 예입니다. *"내일 미팅 있는 회사들, 사전 브리프 좀 만들어줘"* 한 문장으로 **캘린더가 미팅을 찾고 → 3-1의 기업 공시(Custom MCP 서버)로 각 회사를 분석 → Drive에 정리본 저장**까지 한 번에. (3-1에서 단독으로 쓴 Custom MCP를, 이번엔 캘린더·드라이브와 **묶어서** 자동화하는 거예요.)
...
3. 진짜 가치는 **업무에 활용하여 생산성을 높이는 것** — 한 명이 만든 에이전트를 **팀 전체가 같은 품질로** 사용.
...
**커넥터 체험 · 기업 공시 MCP (2-2)**
```
엔비디아 최근 공시 알려줘. 날짜랑 유형도 같이.
```

**Made by Google · Deep Research (3-0, 시연·개인 실습용)**
```
우리 산업의 생성형 AI 도입 동향을 조사해서,
최근 사례와 시사점을 리포트로 정리해줘.
```

**Made by Google · Idea Generation (3-0, 시연·개인 실습용)**
```
사내 직원 생산성을 높일 AI 에이전트 아이디어를 제안해줘.
기준: 2주 안에 시범 적용 가능하고, 기존 사내 시스템과 잘 붙을 것.
```

**Made by Google · NotebookLM (3-0, 시연·개인 실습용)** *(samples/ 자료를 소스로 넣은 뒤)*
```
출장비 한도와 정산 기한을 알려줘. (답변 하단의 인용 출처 확인)
```
