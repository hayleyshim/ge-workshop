...
> 🎯 **두 도구를 다 써 봤어요** — 📚 **사내 지식**(우리 문서에서 찾기)과 🧩 **기업 공시**(외부 시스템에서 실시간으로 가져오기). 3번에서는 이 도구들을 **에이전트에 연결**해, 매번 켜지않아도 알아서 골라 쓰는 **나만의 비서**를 만듭니다.
...
### 3-2. 미팅 사전 브리핑 에이전트 — 여러 도구 활용해보기

> **여러 도구를 한 에이전트에 묶으면** 어떤 일이 벌어지는지 보여주는 예입니다. *"내일 미팅 있는 회사들, 사전 브리프 좀 만들어줘"* 한 문장으로 **캘린더가 미팅을 찾고 → 3-1의 기업 공시(Custom MCP 서버)로 각 회사를 분석 → Drive에 정리본 저장**까지 한 번에. (3-1에서 단독으로 쓴 Custom MCP를, 이번엔 캘린더·드라이브와 **묶어서** 자동화하는 거예요.)
